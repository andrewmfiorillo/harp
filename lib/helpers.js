...
var setup = exports.setup = function(projectPath, env){
  ...
  // console.log("Pre Contents: " + contents);
  ...
  // console.log("JSON parsing: " + contents);
 ...
 *
 *
 * Defaults to terraform's `shouldIgnore` but allows for site-specific configuration
 *
 */

exports.shouldIgnore = function(info, filePath) {
  if (info && info.config && info.config.config && info.config.config.ignore_prefixes) {
    // get the list of prefixes
    var filt = info.config.config.ignore_prefixes;

    // create array out of path
    var arr = filePath.split(path.sep)

    // test if any prefixes are in any part of the path
    var map = arr.map(function(item){
      var match = filt.map(function(prefix){
        item.startsWith(prefix);
      });
      return match.indexOf(true) !== -1;
    })
    ...
    // use the default
    return terraform.helpers.shouldIgnore(filePath);
  }
}

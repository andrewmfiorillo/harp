...
exports.server = function(dirPath, options, callback){
  var app = connect()
  app.use(middleware.setup(dirPath))
  // // logging Middleware
  // app.use(middleware.debuggingLogs);
...
exports.multihost = function(dirPath, options, callback){
  ...
  app.listen(options.port || 9000, options.ip, function(){
    app.projectPath = dirPath;
    callback.apply(app, arguments);
  })
}
...
exports.compile = function(projectPath, outputPath, callback){
  ...
  var copyFile = function(file, done){
    ...
    if(!helpers.shouldIgnore(setup, file) && [".jade", ".ejs", ".md", ".styl", ".less", ".scss", ".sass", ".coffee"].indexOf(ext) === -1){
